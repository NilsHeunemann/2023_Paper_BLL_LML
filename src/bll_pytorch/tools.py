...
class Scaler(nn.Module):
    ...
    def __init__(self, X=None, Y=None):
        ...
        if X is not None:
            self.scaler_x = TorchScaler().fit(X)
            self.n_x = X.shape[1]

        if Y is not None:
            self.scaler_y = TorchScaler().fit(Y)
            self.n_y = Y.shape[1]
...
def blkdiag(*args):
    ...
    if len(args) == 2:
        ...
        assert isinstance(A, torch.Tensor), "Argument must be a numpy.ndarray."
        assert A.ndim == 2, "Tensor must be 2-dimensional"
        assert isinstance(B, torch.Tensor), "Argument must be a numpy.ndarray."
        assert B.ndim == 2, "Tensor must be 2-dimensional"
...
def print_percent_done(
    epoch, total, loss=None, val_loss=None, bar_len=50, title="Please wait"
):
    """
...
def plot_alpha_search(result_train, result_test, alpha_opt, alpha_max, **kwargs):
    ...
    for i, score in enumerate(scores_test):
        ...
        ax_twins[-1].plot(
            log_alpha_train, result_train[score], color=color[1], label="train"
        )
    ...
    ax[0].legend()
    ax[0].set_title(r"Grid search for $\log(\alpha)$")
